...
TARGET_VERSION = "95"
OUTPUT_NAME = "bot_wallapop_profesional_v95_ADMIN_ROTACION_PRO.py"
...
# =========================================================
# V95 · CAPA DE FLUIDEZ DE INTERFAZ
# =========================================================
...
try:
    ...
    print("[V95] Capa de fluidez dashboard no aplicada:", _e_v89_ui)

"""


def _inyectar_fluidez(texto):
    if "V95 · CAPA DE FLUIDEZ DE INTERFAZ" in texto:
        return texto
...
def _hacer_comprobacion_update_inicio_no_bloqueante(texto):
    ...
    if inicio == -1 or fin == -1:
        return texto
    nuevo = """# Comprobación silenciosa del servidor de actualizaciones (V95, no bloqueante).
...
''',
        '''            (
                ...
                "V95 ya migró automáticamente los publicados.json antiguos. "
...
'''
    ...
    try:
        _diag_log_path = Path(CARPETA_DATOS_USUARIO) / "rotacion_v95.log"
...
def _inyectar_motor_rotacion_v95(texto):
    if "V95 · MOTOR DE ROTACIÓN ROBUSTO" in texto:
        return texto

    codigo = r'''

# =========================================================
# V95 · MOTOR DE ROTACIÓN ROBUSTO
# =========================================================
# Objetivo:
...
_ROTACION_V95_LOCK = threading.Lock()
_ROTACION_V95_PROCESADOS = set()
_ROTACION_V95_ULTIMO = ""
_ROTACION_V95_EN_CURSO = False


def _v95_log_rotacion(texto):
    global _ROTACION_V95_ULTIMO

    _ROTACION_V95_ULTIMO = str(
        texto
    )
...
def _v95_titulo_candidato(item):
    if not isinstance(
        item,
        dict
    ):
...
def _v95_click_visible(page, textos, timeout=2500):
    for texto in textos:
        # Texto visible.
        try:
...
def _v95_normalizar_texto(texto):
    try:
        import unicodedata
...
def _v95_guardar_publicados_compat(datos):
    """
...
def _v95_guardar_url_candidato(candidato, url):
    if not url:
        return
    ...
    try:
        ...
        if (
            ...
            _v95_guardar_publicados_compat(
                datos
            )

            return

        titulo_objetivo = _v95_normalizar_texto(
            _v95_titulo_candidato(
                candidato
            )
        )
...
def _v95_extraer_url_por_titulo(page, titulo):
    objetivo = _v95_normalizar_texto(
        titulo
    )
...
def _v95_descubrir_url_candidato(contexto, candidato):
    titulo = _v95_titulo_candidato(
        candidato
    )
    ...
    try:
        for pagina in list(
            ...
            try:
                url = _v95_extraer_url_por_titulo(
                    pagina,
                    titulo
                )

                if url:
                    _v95_guardar_url_candidato(
                        candidato,
                        url
                    )
...
def _v95_abrir_candidato(page, candidato):
    url = _v95_url_candidato(
        candidato
    )
    ...
    if not url:
        try:
            url = _v95_descubrir_url_candidato(
                page.context,
                candidato
            )
        except Exception as e:
...
def _v95_eliminar_con_motor_existente(candidato):
    """
...
def _v95_dump_controles(page, limite=120):
    """
...
def _v95_click_eliminar_avanzado(page):
    patrones = (
        "eliminar",
        "borrar",
        "delete",
...
def _v95_eliminar_en_wallapop(candidato):
    titulo = _v95_titulo_candidato(
        candidato
    )
    ...
    ok_interno, detalle_interno = _v95_eliminar_con_motor_existente(
        candidato
    )
    ...
    try:
        with sync_playwright() as p:
            ...
            eliminado_click, eliminado_texto = _v95_click_eliminar_avanzado(
                pagina
            )

            if not eliminado_click:
                controles = _v95_dump_controles(
                    pagina
                )
...
def _v95_marcar_local_eliminado(candidato):
    try:
        datos = cargar_publicados()
...
def _v95_rotar_una_vez():
    if not _v95_rotacion_activa():
        return False, (
            "Rotación desactivada."
        )

    candidatos = _v95_candidatos_locales()
    ...
    ok, detalle = _v95_eliminar_en_wallapop(
        candidato
    )
...
def _v95_reactivar_item(item, detalle):
    try:
        item[
            "estado_cola"
        ] = "PENDIENTE"
...
def _v95_worker_limite(item, clave):
    global _ROTACION_V95_EN_CURSO

    if not _ROTACION_V95_LOCK.acquire(
        blocking=False
    ):
        return

    _ROTACION_V95_EN_CURSO = True
    ...
    try:
        ...
        try:
            _ROTACION_V95_LOCK.release()
        except Exception:
            pass


def _v95_vigilar_limite_catalogo():
    try:
        if (
            _v95_rotacion_activa()
            and not _ROTACION_V95_EN_CURSO
        ):
            for indice, item in enumerate(
                ...
                if clave in _ROTACION_V95_PROCESADOS:
                    continue

                try:
                    item[
                        "rotacion_detalle"
                    ] = "V95: límite detectado; buscando candidato seguro..."
...
except Exception as _e_v95_rot:
    print(
        "[ROTACION V95] No pude iniciar watchdog:",
        _e_v95_rot
    )
...
'''
    ...
    if marcador not in texto:
        raise RuntimeError(
            "No pude insertar el motor V95 antes del mainloop."
        )
...
def _inyectar_diagnostico_runtime_v95(texto):
    if "V95 · DIAGNÓSTICO RUNTIME GARANTIZADO" in texto:
        return texto

    codigo = r'''

# =========================================================
# V95 · DIAGNÓSTICO RUNTIME GARANTIZADO
# =========================================================
...
def _v95_serializar_seguro(valor):
    try:
        if isinstance(
            valor,
            (
...
def _v95_candidatos_resumen():
    candidatos = []

    try:
        funcion = globals().get(
            "_v95_candidatos_locales"
        )
...
def _v95_abrir_informe_rotacion(texto_base=""):
    try:
        ...
        partes = [
            ...
            "",
            "================ CANDIDATOS ================",
            _v95_candidatos_resumen(),
            "",
            "================ COLA / ERRORES SIN CORTAR ================",
            _v95_texto_cola_completo(),
            "",
            "================ LOG ROTACIÓN ================",
            _v95_log_rotacion_texto(),
            "",
            "================ SISTEMA ================",
            "Chrome/CDP: "
            + str(
...
def _v95_showinfo(title, message, *args, **kwargs):
    try:
        titulo = str(
            title
        ).lower()
        ...
        if (
            ...
            try:
                ventana.after(
                    0,
                    lambda m=mensaje: _v95_abrir_informe_rotacion(
                        m
                    )
                )
...
messagebox.showinfo = _v95_showinfo
...
def construir_v95(fuente_path, texto):
    ...
    nuevo = _inyectar_diagnostico_runtime_v95(nuevo)
    ...
    nuevo = _inyectar_motor_rotacion_v95(nuevo)
...
def main():
    ...
    nuevo = construir_v95(fuente, texto)
