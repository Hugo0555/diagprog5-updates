...
TARGET_VERSION = "92"
OUTPUT_NAME = "bot_wallapop_profesional_v92_ADMIN_DIAGNOSTICO_TOTAL.py"
...
# =========================================================
# V92 · CAPA DE FLUIDEZ DE INTERFAZ
# =========================================================
...
try:
    ...
    print("[V92] Capa de fluidez dashboard no aplicada:", _e_v89_ui)

"""


def _inyectar_fluidez(texto):
    if "V92 · CAPA DE FLUIDEZ DE INTERFAZ" in texto:
        return texto
...
def _hacer_comprobacion_update_inicio_no_bloqueante(texto):
    ...
    if inicio == -1 or fin == -1:
        return texto
    nuevo = """# Comprobación silenciosa del servidor de actualizaciones (V92, no bloqueante).
...
''',
        '''            (
                ...
                "V92 ya migró automáticamente los publicados.json antiguos. "
...
'''
    ...
    try:
        _diag_log_path = Path(CARPETA_DATOS_USUARIO) / "rotacion_v92.log"
...
def _inyectar_motor_rotacion_v92(texto):
    if "V92 · MOTOR DE ROTACIÓN ROBUSTO" in texto:
        return texto

    codigo = r'''

# =========================================================
# V92 · MOTOR DE ROTACIÓN ROBUSTO
# =========================================================
# Objetivo:
...
_ROTACION_V92_LOCK = threading.Lock()
_ROTACION_V92_PROCESADOS = set()
_ROTACION_V92_ULTIMO = ""
_ROTACION_V92_EN_CURSO = False


def _v92_log_rotacion(texto):
    global _ROTACION_V92_ULTIMO

    _ROTACION_V92_ULTIMO = str(
        texto
    )
...
def _v92_titulo_candidato(item):
    if not isinstance(
        item,
        dict
    ):
...
def _v92_click_visible(page, textos, timeout=2500):
    for texto in textos:
        # Texto visible.
        try:
...
def _v92_abrir_candidato(page, candidato):
    url = _v92_url_candidato(
        candidato
    )

    titulo = _v92_titulo_candidato(
        candidato
    )
...
def _v92_eliminar_con_motor_existente(candidato):
    """
...
def _v92_eliminar_en_wallapop(candidato):
    titulo = _v92_titulo_candidato(
        candidato
    )
    ...
    ok_interno, detalle_interno = _v92_eliminar_con_motor_existente(
        candidato
    )
    ...
    try:
        with sync_playwright() as p:
            ...
            eliminado_click = _v92_click_visible(
                pagina,
                (
                    "Eliminar producto",
                    "Eliminar anuncio",
...
def _v92_marcar_local_eliminado(candidato):
    try:
        datos = cargar_publicados()
...
def _v92_rotar_una_vez():
    if not _v92_rotacion_activa():
        return False, (
            "Rotación desactivada."
        )

    candidatos = _v92_candidatos_locales()
    ...
    ok, detalle = _v92_eliminar_en_wallapop(
        candidato
    )
...
def _v92_reactivar_item(item, detalle):
    try:
        item[
            "estado_cola"
        ] = "PENDIENTE"
...
def _v92_worker_limite(item, clave):
    global _ROTACION_V92_EN_CURSO

    if not _ROTACION_V92_LOCK.acquire(
        blocking=False
    ):
        return

    _ROTACION_V92_EN_CURSO = True
    ...
    try:
        ...
        try:
            _ROTACION_V92_LOCK.release()
        except Exception:
            pass


def _v92_vigilar_limite_catalogo():
    try:
        if (
            _v92_rotacion_activa()
            and not _ROTACION_V92_EN_CURSO
        ):
            for indice, item in enumerate(
                ...
                if clave in _ROTACION_V92_PROCESADOS:
                    continue

                try:
                    item[
                        "rotacion_detalle"
                    ] = "V92: límite detectado; buscando candidato seguro..."
...
except Exception as _e_v92_rot:
    print(
        "[ROTACION V92] No pude iniciar watchdog:",
        _e_v92_rot
    )
...
'''
    ...
    if marcador not in texto:
        raise RuntimeError(
            "No pude insertar el motor V92 antes del mainloop."
        )
...
def construir_v92(fuente_path, texto):
    ...
    nuevo = _inyectar_motor_rotacion_v92(nuevo)
...
def main():
    ...
    nuevo = construir_v92(fuente, texto)
