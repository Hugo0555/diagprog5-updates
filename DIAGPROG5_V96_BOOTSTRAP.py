...
TARGET_VERSION = "96"
OUTPUT_NAME = "bot_wallapop_profesional_v96_ADMIN_CAPACIDAD_COLA.py"
...
# =========================================================
# V96 · CAPA DE FLUIDEZ DE INTERFAZ
# =========================================================
...
try:
    ...
    print("[V96] Capa de fluidez dashboard no aplicada:", _e_v89_ui)

"""


def _inyectar_fluidez(texto):
    if "V96 · CAPA DE FLUIDEZ DE INTERFAZ" in texto:
        return texto
...
def _hacer_comprobacion_update_inicio_no_bloqueante(texto):
    ...
    if inicio == -1 or fin == -1:
        return texto
    nuevo = """# Comprobación silenciosa del servidor de actualizaciones (V96, no bloqueante).
...
''',
        '''            (
                ...
                "V96 ya migró automáticamente los publicados.json antiguos. "
...
'''
    ...
    try:
        _diag_log_path = Path(CARPETA_DATOS_USUARIO) / "rotacion_v96.log"
...
def _inyectar_motor_rotacion_v96(texto):
    if "V96 · MOTOR DE ROTACIÓN ROBUSTO" in texto:
        return texto

    codigo = r'''

# =========================================================
# V96 · MOTOR DE ROTACIÓN ROBUSTO
# =========================================================
# Objetivo:
...
_ROTACION_V96_LOCK = threading.Lock()
_ROTACION_V96_PROCESADOS = set()
_ROTACION_V96_ULTIMO = ""
_ROTACION_V96_EN_CURSO = False


def _v96_log_rotacion(texto):
    global _ROTACION_V96_ULTIMO

    _ROTACION_V96_ULTIMO = str(
        texto
    )
...
def _v96_titulo_candidato(item):
    if not isinstance(
        item,
        dict
    ):
...
def _v96_click_visible(page, textos, timeout=2500):
    for texto in textos:
        # Texto visible.
        try:
...
def _v96_normalizar_texto(texto):
    try:
        import unicodedata
...
def _v96_guardar_publicados_compat(datos):
    """
...
def _v96_guardar_url_candidato(candidato, url):
    if not url:
        return
    ...
    try:
        ...
        if (
            ...
            _v96_guardar_publicados_compat(
                datos
            )

            return

        titulo_objetivo = _v96_normalizar_texto(
            _v96_titulo_candidato(
                candidato
            )
        )
...
def _v96_extraer_url_por_titulo(page, titulo):
    objetivo = _v96_normalizar_texto(
        titulo
    )
...
def _v96_descubrir_url_candidato(contexto, candidato):
    titulo = _v96_titulo_candidato(
        candidato
    )
    ...
    try:
        for pagina in list(
            ...
            try:
                url = _v96_extraer_url_por_titulo(
                    pagina,
                    titulo
                )

                if url:
                    _v96_guardar_url_candidato(
                        candidato,
                        url
                    )
...
def _v96_abrir_candidato(page, candidato):
    url = _v96_url_candidato(
        candidato
    )
    ...
    if not url:
        try:
            url = _v96_descubrir_url_candidato(
                page.context,
                candidato
            )
        except Exception as e:
...
def _v96_eliminar_con_motor_existente(candidato):
    """
...
def _v96_dump_controles(page, limite=120):
    """
...
def _v96_click_eliminar_avanzado(page):
    patrones = (
        "eliminar",
        "borrar",
        "delete",
...
def _v96_eliminar_en_wallapop(candidato):
    titulo = _v96_titulo_candidato(
        candidato
    )
    ...
    ok_interno, detalle_interno = _v96_eliminar_con_motor_existente(
        candidato
    )
    ...
    try:
        with sync_playwright() as p:
            ...
            eliminado_click, eliminado_texto = _v96_click_eliminar_avanzado(
                pagina
            )

            if not eliminado_click:
                controles = _v96_dump_controles(
                    pagina
                )
...
def _v96_marcar_local_eliminado(candidato):
    try:
        datos = cargar_publicados()
...
def _v96_rotar_una_vez():
    if not _v96_rotacion_activa():
        return False, (
            "Rotación desactivada."
        )

    candidatos = _v96_candidatos_locales()
    ...
    ok, detalle = _v96_eliminar_en_wallapop(
        candidato
    )
...
def _v96_reactivar_item(item, detalle):
    try:
        item[
            "estado_cola"
        ] = "PENDIENTE"
...
def _v96_worker_limite(item, clave):
    global _ROTACION_V96_EN_CURSO

    if not _ROTACION_V96_LOCK.acquire(
        blocking=False
    ):
        return

    _ROTACION_V96_EN_CURSO = True
    ...
    try:
        ...
        try:
            _ROTACION_V96_LOCK.release()
        except Exception:
            pass


def _v96_vigilar_limite_catalogo():
    try:
        if (
            _v96_rotacion_activa()
            and not _ROTACION_V96_EN_CURSO
        ):
            for indice, item in enumerate(
                ...
                if clave in _ROTACION_V96_PROCESADOS:
                    continue

                try:
                    item[
                        "rotacion_detalle"
                    ] = "V96: límite detectado; buscando candidato seguro..."
...
except Exception as _e_v96_rot:
    print(
        "[ROTACION V96] No pude iniciar watchdog:",
        _e_v96_rot
    )
...
'''
    ...
    if marcador not in texto:
        raise RuntimeError(
            "No pude insertar el motor V96 antes del mainloop."
        )
...
def _inyectar_diagnostico_runtime_v96(texto):
    if "V96 · DIAGNÓSTICO RUNTIME GARANTIZADO" in texto:
        return texto

    codigo = r'''

# =========================================================
# V96 · DIAGNÓSTICO RUNTIME GARANTIZADO
# =========================================================
...
def _v96_serializar_seguro(valor):
    try:
        if isinstance(
            valor,
            (
...
def _v96_candidatos_resumen():
    candidatos = []

    try:
        funcion = globals().get(
            "_v96_candidatos_locales"
        )
...
def _v96_abrir_informe_rotacion(texto_base=""):
    try:
        ...
        partes = [
            ...
            "",
            "================ CANDIDATOS ================",
            _v96_candidatos_resumen(),
            "",
            "================ COLA / ERRORES SIN CORTAR ================",
            _v96_texto_cola_completo(),
            "",
            "================ LOG ROTACIÓN ================",
            _v96_log_rotacion_texto(),
            "",
            "================ SISTEMA ================",
            "Chrome/CDP: "
            + str(
...
def _v96_showinfo(title, message, *args, **kwargs):
    try:
        titulo = str(
            title
        ).lower()
        ...
        if (
            ...
            try:
                ventana.after(
                    0,
                    lambda m=mensaje: _v96_abrir_informe_rotacion(
                        m
                    )
                )
...
messagebox.showinfo = _v96_showinfo
...
def _inyectar_capacidad_cola_v96(texto):
    if "V96 · CAPACIDAD CONFIGURABLE DE COLA" in texto:
        return texto
...
def _v96_cargar_capacidad_cola():
    try:
        if _ARCHIVO_CAPACIDAD_COLA.exists():
            datos = json.loads(_ARCHIVO_CAPACIDAD_COLA.read_text(encoding="utf-8"))
            return max(_CAPACIDAD_COLA_MIN, min(_CAPACIDAD_COLA_MAX, int(datos.get("capacidad", _CAPACIDAD_COLA_DEFECTO))))
    except Exception:
        pass
    return _CAPACIDAD_COLA_DEFECTO


def _v96_guardar_capacidad_cola(valor):
    valor = max(_CAPACIDAD_COLA_MIN, min(_CAPACIDAD_COLA_MAX, int(valor)))
...
CAPACIDAD_COLA = _v96_cargar_capacidad_cola()


def capacidad_cola_actual():
    return int(globals().get("CAPACIDAD_COLA", _CAPACIDAD_COLA_DEFECTO))


def _v96_refrescar_estado_capacidad():
    try:
    ...
    try:
        _v96_label_capacidad.configure(
            text=f"Uso actual: {actual} / {capacidad_cola_actual()}"
        )
    except Exception:
        pass


def _v96_guardar_capacidad_desde_ui():
    global CAPACIDAD_COLA
    try:
        valor = int(_v96_entry_capacidad.get().strip())
    except Exception:
    ...
    CAPACIDAD_COLA = _v96_guardar_capacidad_cola(valor)
    _v96_refrescar_estado_capacidad()
    messagebox.showinfo("Capacidad de cola", f"Capacidad guardada: {CAPACIDAD_COLA} anuncios.")


def _v96_instalar_control_capacidad():
    global _v96_entry_capacidad, _v96_label_capacidad

    parent = globals().get("queue_card")

    if parent is None:
        print("[V96] No encontré queue_card; no puedo mostrar capacidad dentro de Cola.")
        return

    try:
        panel = ctk.CTkFrame(parent, fg_color="#081017", corner_radius=10)
        panel.pack(fill="x", padx=10, pady=(8, 10))

        ctk.CTkLabel(
            panel,
            text="Capacidad de la cola",
            font=("Segoe UI", 12, "bold")
        ).pack(anchor="w", padx=12, pady=(10, 2))

        ctk.CTkLabel(
            panel,
            text="Elige el máximo de anuncios permitidos en la cola.",
            font=("Segoe UI", 9),
            text_color="#98a7b2"
        ...
        _v96_entry_capacidad = ctk.CTkEntry(fila, height=36)
        _v96_entry_capacidad.pack(side="left", fill="x", expand=True, padx=(0, 8))
        _v96_entry_capacidad.insert(0, str(capacidad_cola_actual()))

        ctk.CTkButton(
            fila,
            text="Guardar capacidad",
            command=_v96_guardar_capacidad_desde_ui,
            height=36
        ).pack(side="left", padx=(0, 8))

        _v96_label_capacidad = ctk.CTkLabel(
            fila,
            text="",
            font=("Segoe UI", 10, "bold")
        )
        _v96_label_capacidad.pack(side="right")

        _v96_refrescar_estado_capacidad()

    except Exception as e:
        print("[V96] Error creando control de capacidad:", e)


def _v96_envuelve_funcion_cola(nombre, funcion):
    if getattr(funcion, "_v96_capacidad_wrapped", False):
        return funcion

    def wrapper(*args, **kwargs):
        try:
            antes = len(cola_anuncios)
        except Exception:
            antes = 0
        ...
        if antes >= limite:
            messagebox.showwarning(
                "Cola llena",
                f"La cola ya tiene {antes}/{limite} anuncios. Aumenta la capacidad para añadir más."
            )
            return None
        ...
        try:
            if len(cola_anuncios) > limite:
                ...
                messagebox.showwarning(
                    "Capacidad alcanzada",
                    f"Se alcanzó el máximo de {limite} anuncios. No se añadieron más elementos."
                )
        except Exception:
            pass
    ...
    wrapper._v96_capacidad_wrapped = True
    wrapper.__name__ = getattr(funcion, "__name__", nombre)
    return wrapper


def _v96_instalar_limite_funciones_cola():
    for nombre, funcion in list(globals().items()):
        ...
        if not any(x in n for x in ("anadir", "añadir", "agregar", "insertar")):
            continue

        if any(x in n for x in ("guardar", "cargar", "refrescar", "iniciar", "procesar", "publicar")):
            continue

        try:
            globals()[nombre] = _v96_envuelve_funcion_cola(nombre, funcion)
        except Exception:
            pass


def _v96_tick_capacidad():
    _v96_refrescar_estado_capacidad()
    try:
        ventana.after(1000, _v96_tick_capacidad)
    except Exception:
        pass


try:
    ventana.after(800, _v96_instalar_control_capacidad)
    ventana.after(1000, _v96_instalar_limite_funciones_cola)
    ventana.after(1200, _v96_tick_capacidad)
except Exception as e:
    print("[V96] No pude iniciar capacidad de cola:", e)
...
def construir_v96(fuente_path, texto):
    ...
    nuevo = _inyectar_motor_rotacion_v96(nuevo)
...
def main():
    ...
    nuevo = construir_v96(fuente, texto)
