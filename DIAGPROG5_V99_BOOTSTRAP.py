...
TARGET_VERSION = "99"
OUTPUT_NAME = "bot_wallapop_profesional_v99_ADMIN_COLA_SIN_LIMITE_10.py"
...
# =========================================================
# V99 · CAPA DE FLUIDEZ DE INTERFAZ
# =========================================================
...
try:
    ...
    print("[V99] Capa de fluidez dashboard no aplicada:", _e_v89_ui)

"""


def _inyectar_fluidez(texto):
    if "V99 · CAPA DE FLUIDEZ DE INTERFAZ" in texto:
        return texto
...
def _hacer_comprobacion_update_inicio_no_bloqueante(texto):
    ...
    if inicio == -1 or fin == -1:
        return texto
    nuevo = """# Comprobación silenciosa del servidor de actualizaciones (V99, no bloqueante).
...
''',
        '''            (
                ...
                "V99 ya migró automáticamente los publicados.json antiguos. "
...
'''
    ...
    try:
        _diag_log_path = Path(CARPETA_DATOS_USUARIO) / "rotacion_v99.log"
...
def _inyectar_motor_rotacion_v99(texto):
    if "V99 · MOTOR DE ROTACIÓN ROBUSTO" in texto:
        return texto

    codigo = r'''

# =========================================================
# V99 · MOTOR DE ROTACIÓN ROBUSTO
# =========================================================
# Objetivo:
...
_ROTACION_V99_LOCK = threading.Lock()
_ROTACION_V99_PROCESADOS = set()
_ROTACION_V99_ULTIMO = ""
_ROTACION_V99_EN_CURSO = False


def _v99_log_rotacion(texto):
    global _ROTACION_V99_ULTIMO

    _ROTACION_V99_ULTIMO = str(
        texto
    )
...
def _v99_titulo_candidato(item):
    if not isinstance(
        item,
        dict
    ):
...
def _v99_click_visible(page, textos, timeout=2500):
    for texto in textos:
        # Texto visible.
        try:
...
def _v99_normalizar_texto(texto):
    try:
        import unicodedata
...
def _v99_guardar_publicados_compat(datos):
    """
...
def _v99_guardar_url_candidato(candidato, url):
    if not url:
        return
    ...
    try:
        ...
        if (
            ...
            _v99_guardar_publicados_compat(
                datos
            )

            return

        titulo_objetivo = _v99_normalizar_texto(
            _v99_titulo_candidato(
                candidato
            )
        )
...
def _v99_extraer_url_por_titulo(page, titulo):
    objetivo = _v99_normalizar_texto(
        titulo
    )
...
def _v99_descubrir_url_candidato(contexto, candidato):
    titulo = _v99_titulo_candidato(
        candidato
    )
    ...
    try:
        for pagina in list(
            ...
            try:
                url = _v99_extraer_url_por_titulo(
                    pagina,
                    titulo
                )

                if url:
                    _v99_guardar_url_candidato(
                        candidato,
                        url
                    )
...
def _v99_abrir_candidato(page, candidato):
    url = _v99_url_candidato(
        candidato
    )
    ...
    if not url:
        try:
            url = _v99_descubrir_url_candidato(
                page.context,
                candidato
            )
        except Exception as e:
...
def _v99_eliminar_con_motor_existente(candidato):
    """
...
def _v99_dump_controles(page, limite=120):
    """
...
def _v99_click_eliminar_avanzado(page):
    patrones = (
        "eliminar",
        "borrar",
        "delete",
...
def _v99_eliminar_en_wallapop(candidato):
    titulo = _v99_titulo_candidato(
        candidato
    )
    ...
    ok_interno, detalle_interno = _v99_eliminar_con_motor_existente(
        candidato
    )
    ...
    try:
        with sync_playwright() as p:
            ...
            eliminado_click, eliminado_texto = _v99_click_eliminar_avanzado(
                pagina
            )

            if not eliminado_click:
                controles = _v99_dump_controles(
                    pagina
                )
...
def _v99_marcar_local_eliminado(candidato):
    try:
        datos = cargar_publicados()
...
def _v99_rotar_una_vez():
    if not _v99_rotacion_activa():
        return False, (
            "Rotación desactivada."
        )

    candidatos = _v99_candidatos_locales()
    ...
    ok, detalle = _v99_eliminar_en_wallapop(
        candidato
    )
...
def _v99_reactivar_item(item, detalle):
    try:
        item[
            "estado_cola"
        ] = "PENDIENTE"
...
def _v99_worker_limite(item, clave):
    global _ROTACION_V99_EN_CURSO

    if not _ROTACION_V99_LOCK.acquire(
        blocking=False
    ):
        return

    _ROTACION_V99_EN_CURSO = True
    ...
    try:
        ...
        try:
            _ROTACION_V99_LOCK.release()
        except Exception:
            pass


def _v99_vigilar_limite_catalogo():
    try:
        if (
            _v99_rotacion_activa()
            and not _ROTACION_V99_EN_CURSO
        ):
            for indice, item in enumerate(
                ...
                if clave in _ROTACION_V99_PROCESADOS:
                    continue

                try:
                    item[
                        "rotacion_detalle"
                    ] = "V99: límite detectado; buscando candidato seguro..."
...
except Exception as _e_v99_rot:
    print(
        "[ROTACION V99] No pude iniciar watchdog:",
        _e_v99_rot
    )
...
'''
    ...
    if marcador not in texto:
        raise RuntimeError(
            "No pude insertar el motor V99 antes del mainloop."
        )
...
def _inyectar_diagnostico_runtime_v99(texto):
    if "V99 · DIAGNÓSTICO RUNTIME GARANTIZADO" in texto:
        return texto

    codigo = r'''

# =========================================================
# V99 · DIAGNÓSTICO RUNTIME GARANTIZADO
# =========================================================
...
def _v99_serializar_seguro(valor):
    try:
        if isinstance(
            valor,
            (
...
def _v99_candidatos_resumen():
    candidatos = []

    try:
        funcion = globals().get(
            "_v99_candidatos_locales"
        )
...
def _v99_abrir_informe_rotacion(texto_base=""):
    try:
        ...
        partes = [
            ...
            "",
            "================ CANDIDATOS ================",
            _v99_candidatos_resumen(),
            "",
            "================ COLA / ERRORES SIN CORTAR ================",
            _v99_texto_cola_completo(),
            "",
            "================ LOG ROTACIÓN ================",
            _v99_log_rotacion_texto(),
            "",
            "================ SISTEMA ================",
            "Chrome/CDP: "
            + str(
...
def _v99_showinfo(title, message, *args, **kwargs):
    try:
        titulo = str(
            title
        ).lower()
        ...
        if (
            ...
            try:
                ventana.after(
                    0,
                    lambda m=mensaje: _v99_abrir_informe_rotacion(
                        m
                    )
                )
...
messagebox.showinfo = _v99_showinfo
...
def _inyectar_capacidad_cola_v99(texto):
    if "V99 · CAPACIDAD VISIBLE EN BARRA DE COLA" in texto:
        return texto
...
_v99_boton_capacidad = None
_v99_control_instalado = False


def _v99_cargar_capacidad():
    try:
...
def _v99_guardar_capacidad(valor):
...
CAPACIDAD_COLA = _v99_cargar_capacidad()
...
def _v99_actualizar_boton():
    try:
        if _v99_boton_capacidad is not None:
            _v99_boton_capacidad.configure(
                text=f"⚙ Capacidad {_v99_uso_cola()}/{capacidad_cola_actual()}"
            )
    except Exception:
        pass


def _v99_abrir_dialogo_capacidad():
    ...
    def guardar():
        ...
        CAPACIDAD_COLA = _v99_guardar_capacidad(valor)
        _v99_actualizar_boton()
        dialogo.destroy()
...
def _v99_descendientes(widget):
    ...
    for hijo in hijos:
        salida.append(hijo)
        salida.extend(_v99_descendientes(hijo))
    return salida


def _v99_texto(widget):
    try:
...
def _v99_buscar_boton_anadir():
    try:
        todos = _v99_descendientes(ventana)
    except Exception:
        todos = []

    for widget in todos:
        texto = _v99_texto(widget)
        if (
...
def _v99_instalar_boton_capacidad(intentos=0):
    global _v99_boton_capacidad, _v99_control_instalado

    if _v99_control_instalado:
        return

    referencia = _v99_buscar_boton_anadir()

    if referencia is None:
        if intentos < 60:
            ventana.after(
                500,
                lambda: _v99_instalar_boton_capacidad(intentos + 1)
            )
        return
    ...
    try:
        ...
        _v99_boton_capacidad = ctk.CTkButton(
            parent,
            text="",
            command=_v99_abrir_dialogo_capacidad,
            height=32,
            width=155,
            fg_color="#245a86",
            hover_color="#2f6fa3"
        ...
        if gestor == "grid":
            ...
            _v99_boton_capacidad.pack(
                side="left",
                padx=(8, 0)
            )

        _v99_control_instalado = True
        _v99_actualizar_boton()

    except Exception as e:
        print("[V99] Error instalando botón capacidad:", e)
        if intentos < 60:
            ventana.after(
                500,
                lambda: _v99_instalar_boton_capacidad(intentos + 1)
...
def _v99_envuelve_funcion_cola(nombre, funcion):
    if getattr(funcion, "_v99_capacidad_wrapped", False):
        return funcion

    def wrapper(*args, **kwargs):
        actual = _v99_uso_cola()
    ...
    wrapper._v99_capacidad_wrapped = True
    wrapper.__name__ = getattr(funcion, "__name__", nombre)
    return wrapper


def _v99_instalar_limites():
    for nombre, funcion in list(globals().items()):
        ...
        try:
            globals()[nombre] = _v99_envuelve_funcion_cola(nombre, funcion)
        except Exception:
            pass
...
def _v99_tick():
    ...
    if not _v99_control_instalado:
        _v99_instalar_boton_capacidad()

    try:
        ventana.after(1000, _v99_tick)
    except Exception:
        pass


try:
    ventana.after(300, _v99_instalar_boton_capacidad)
    ventana.after(700, _v99_instalar_limites)
    ventana.after(1000, _v99_tick)
except Exception as e:
    print("[V99] No pude iniciar capacidad de cola:", e)
...
def construir_v99(fuente_path, texto):
    nuevo = _limpiar_motores_rotacion_generados(
        texto
    )
    nuevo = _reemplazar_version(nuevo)
    nuevo = _inyectar_capacidad_cola_v99(nuevo)
    ...
    nuevo = _inyectar_diagnostico_runtime_v99(nuevo)
    ...
    nuevo = _inyectar_motor_rotacion_v99(nuevo)
...
def main():
    ...
    nuevo = construir_v99(fuente, texto)
    ...
    if proceso_nuevo.poll() is not None:
        raise RuntimeError(
            "La V99 se cerró durante el arranque. "
