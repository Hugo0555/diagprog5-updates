...
TARGET_VERSION = "100"
OUTPUT_NAME = "bot_wallapop_profesional_v100_ADMIN_CONSOLIDADO.py"
...
# =========================================================
# V100 · CAPA DE FLUIDEZ DE INTERFAZ
# =========================================================
...
try:
    ...
    print("[V100] Capa de fluidez dashboard no aplicada:", _e_v89_ui)

"""


def _inyectar_fluidez(texto):
    if "V100 · CAPA DE FLUIDEZ DE INTERFAZ" in texto:
        return texto
...
'''
    ...
    try:
        _diag_log_path = Path(CARPETA_DATOS_USUARIO) / "rotacion_v100.log"
...
def _inyectar_motor_rotacion_v100(texto):
    if "V100 · MOTOR DE ROTACIÓN ROBUSTO" in texto:
        return texto

    codigo = r'''

# =========================================================
# V100 · MOTOR DE ROTACIÓN ROBUSTO
# =========================================================
# Objetivo:
...
_ROTACION_V100_LOCK = threading.Lock()
_ROTACION_V100_PROCESADOS = set()
_ROTACION_V100_ULTIMO = ""
_ROTACION_V100_EN_CURSO = False


def _v100_log_rotacion(texto):
    global _ROTACION_V100_ULTIMO

    _ROTACION_V100_ULTIMO = str(
        texto
    )
...
def _v100_titulo_candidato(item):
    if not isinstance(
        item,
        dict
    ):
...
def _v100_click_visible(page, textos, timeout=2500):
    for texto in textos:
        # Texto visible.
        try:
...
def _v100_normalizar_texto(texto):
    try:
        import unicodedata
...
def _v100_guardar_publicados_compat(datos):
    """
...
def _v100_guardar_url_candidato(candidato, url):
    if not url:
        return
    ...
    try:
        ...
        if (
            ...
            _v100_guardar_publicados_compat(
                datos
            )

            return

        titulo_objetivo = _v100_normalizar_texto(
            _v100_titulo_candidato(
                candidato
            )
        )
...
def _v100_extraer_url_por_titulo(page, titulo):
    objetivo = _v100_normalizar_texto(
        titulo
    )
    ...
    try:
        ...
        for i in range(
            ...
            try:
                ...
                texto_n = _v100_normalizar_texto(
                    texto_anchor
                )
...
def _v100_descubrir_url_candidato(contexto, candidato):
    titulo = _v100_titulo_candidato(
        candidato
    )
    ...
    try:
        for pagina in list(
            ...
            try:
                url = _v100_extraer_url_por_titulo(
                    pagina,
                    titulo
                )

                if url:
                    _v100_guardar_url_candidato(
                        candidato,
                        url
                    )
...
def _v100_abrir_candidato(page, candidato):
    url = _v100_url_candidato(
        candidato
    )
    ...
    if not url:
        try:
            url = _v100_descubrir_url_candidato(
                page.context,
                candidato
            )
        except Exception as e:
...
def _v100_eliminar_con_motor_existente(candidato):
    """
...
def _v100_dump_controles(page, limite=120):
    """
...
def _v100_click_eliminar_avanzado(page):
    patrones = (
        "eliminar",
        "borrar",
        "delete",
...
def _v100_eliminar_en_wallapop(candidato):
    titulo = _v100_titulo_candidato(
        candidato
    )
    ...
    ok_interno, detalle_interno = _v100_eliminar_con_motor_existente(
        candidato
    )
    ...
    try:
        with sync_playwright() as p:
            ...
            eliminado_click, eliminado_texto = _v100_click_eliminar_avanzado(
                pagina
            )

            if not eliminado_click:
                controles = _v100_dump_controles(
                    pagina
                )
            ...
            # Confirmación.
            confirmado = _v100_click_visible(
                pagina,
                (
                    "Sí, eliminar",
...
def _v100_marcar_local_eliminado(candidato):
    try:
        datos = cargar_publicados()
...
def _v100_rotar_una_vez():
    if not _v100_rotacion_activa():
        return False, (
            "Rotación desactivada."
        )

    candidatos = _v100_candidatos_locales()
    ...
    ok, detalle = _v100_eliminar_en_wallapop(
        candidato
    )
...
def _v100_reactivar_item(item, detalle):
    try:
        item[
            "estado_cola"
        ] = "PENDIENTE"
...
def _v100_worker_limite(item, clave):
    global _ROTACION_V100_EN_CURSO

    if not _ROTACION_V100_LOCK.acquire(
        blocking=False
    ):
        return

    _ROTACION_V100_EN_CURSO = True
    ...
    try:
        ...
        ok, detalle = _v100_rotar_una_vez()

        if ok:
            _v100_reactivar_item(
                item,
                detalle
            )
        ...
        try:
            _ROTACION_V100_LOCK.release()
        except Exception:
            pass


def _v100_vigilar_limite_catalogo():
    try:
        if (
            _v100_rotacion_activa()
            and not _ROTACION_V100_EN_CURSO
        ):
            for indice, item in enumerate(
                ...
                if clave in _ROTACION_V100_PROCESADOS:
                    continue

                try:
                    item[
                        "rotacion_detalle"
                    ] = "V100: límite detectado; buscando candidato seguro..."
...
except Exception as _e_v100_rot:
    print(
        "[ROTACION V100] No pude iniciar watchdog:",
        _e_v100_rot
    )
...
'''
    ...
    if marcador not in texto:
        raise RuntimeError(
            "No pude insertar el motor V100 antes del mainloop."
        )
...
def _inyectar_diagnostico_runtime_v100(texto):
    if "V100 · DIAGNÓSTICO RUNTIME GARANTIZADO" in texto:
        return texto

    codigo = r'''

# =========================================================
# V100 · DIAGNÓSTICO RUNTIME GARANTIZADO
# =========================================================
...
def _v100_serializar_seguro(valor):
    try:
        if isinstance(
            valor,
            (
...
def _v100_candidatos_resumen():
    candidatos = []

    try:
        funcion = globals().get(
            "_v100_candidatos_locales"
        )
...
def _v100_abrir_informe_rotacion(texto_base=""):
    try:
        ...
        partes = [
            ...
            "",
            "================ CANDIDATOS ================",
            _v100_candidatos_resumen(),
            "",
            "================ COLA / ERRORES SIN CORTAR ================",
            _v100_texto_cola_completo(),
            "",
            "================ LOG ROTACIÓN ================",
            _v100_log_rotacion_texto(),
            "",
            "================ SISTEMA ================",
            "Chrome/CDP: "
            + str(
...
def _v100_showinfo(title, message, *args, **kwargs):
    try:
        titulo = str(
            title
        ).lower()
        ...
        if (
            ...
            try:
                ventana.after(
                    0,
                    lambda m=mensaje: _v100_abrir_informe_rotacion(
                        m
                    )
                )
...
messagebox.showinfo = _v100_showinfo
...
def _inyectar_capacidad_cola_v100(texto):
    if "V100 · CAPACIDAD VISIBLE EN BARRA DE COLA" in texto:
        return texto
...
_v100_boton_capacidad = None
_v100_control_instalado = False


def _v100_cargar_capacidad():
    try:
...
def _v100_guardar_capacidad(valor):
...
CAPACIDAD_COLA = _v100_cargar_capacidad()
...
def _v100_actualizar_boton():
    try:
        if _v100_boton_capacidad is not None:
            _v100_boton_capacidad.configure(
                text=f"⚙ Capacidad {_v100_uso_cola()}/{capacidad_cola_actual()}"
            )
    except Exception:
        pass


def _v100_abrir_dialogo_capacidad():
    ...
    def guardar():
        ...
        CAPACIDAD_COLA = _v100_guardar_capacidad(valor)
        _v100_actualizar_boton()
        dialogo.destroy()
...
def _v100_descendientes(widget):
    salida = []
    try:
    ...
    for hijo in hijos:
        salida.append(hijo)
        salida.extend(_v100_descendientes(hijo))
    return salida


def _v100_texto(widget):
    try:
...
def _v100_buscar_boton_anadir():
    try:
        todos = _v100_descendientes(ventana)
    except Exception:
        todos = []

    for widget in todos:
        texto = _v100_texto(widget)
        if (
...
def _v100_instalar_boton_capacidad(intentos=0):
    global _v100_boton_capacidad, _v100_control_instalado

    if _v100_control_instalado:
        return

    referencia = _v100_buscar_boton_anadir()

    if referencia is None:
        if intentos < 60:
            ventana.after(
                500,
                lambda: _v100_instalar_boton_capacidad(intentos + 1)
            )
        return
    ...
    try:
        ...
        _v100_boton_capacidad = ctk.CTkButton(
            parent,
            text="",
            command=_v100_abrir_dialogo_capacidad,
            height=32,
            width=155,
            fg_color="#245a86",
            hover_color="#2f6fa3"
        ...
        if gestor == "grid":
            ...
            _v100_boton_capacidad.pack(
                side="left",
                padx=(8, 0)
            )

        _v100_control_instalado = True
        _v100_actualizar_boton()

    except Exception as e:
        print("[V100] Error instalando botón capacidad:", e)
        if intentos < 60:
            ventana.after(
                500,
                lambda: _v100_instalar_boton_capacidad(intentos + 1)
...
def _v100_envuelve_funcion_cola(nombre, funcion):
    if getattr(funcion, "_v100_capacidad_wrapped", False):
        return funcion

    def wrapper(*args, **kwargs):
        actual = _v100_uso_cola()
    ...
    wrapper._v100_capacidad_wrapped = True
    wrapper.__name__ = getattr(funcion, "__name__", nombre)
    return wrapper


def _v100_instalar_limites():
    for nombre, funcion in list(globals().items()):
        ...
        try:
            globals()[nombre] = _v100_envuelve_funcion_cola(nombre, funcion)
        except Exception:
            pass
...
def _v100_tick():
    ...
    if not _v100_control_instalado:
        _v100_instalar_boton_capacidad()

    try:
        ventana.after(1000, _v100_tick)
    except Exception:
        pass


try:
    ventana.after(300, _v100_instalar_boton_capacidad)
    ventana.after(700, _v100_instalar_limites)
    ventana.after(1000, _v100_tick)
except Exception as e:
    print("[V100] No pude iniciar capacidad de cola:", e)
...
def _consolidar_capacidad_v100(texto):
    """
...
''',
    ...
    viejo_guardar = '''def _v100_guardar_capacidad(valor):
    valor = max(1, min(5000, int(valor)))
    _ARCHIVO_CAPACIDAD_COLA.parent.mkdir(parents=True, exist_ok=True)
    _ARCHIVO_CAPACIDAD_COLA.write_text(
        json.dumps({"capacidad": valor}, ensure_ascii=False, indent=2),
...
'''
    ...
    nuevo_guardar = '''def _v100_guardar_capacidad(valor):
    global MAX_COLA
...
'''
    ...
    texto = texto.replace(
        'f"⚙ Capacidad {_v100_uso_cola()}/{capacidad_cola_actual()}"',
        'f"⚙ Capacidad {_v100_uso_cola()}/{limite_cola_efectivo()}"'
    )
    texto = texto.replace(
        "limite = capacidad_cola_actual()",
...
'''
    ...
    marcador_diag = '''def _v100_tick():
'''

    if marcador_diag in texto and "def _v100_capacidad_diagnostico():" not in texto:
        texto = texto.replace(
            marcador_diag,
            '''def _v100_capacidad_diagnostico():
    return {
...
def construir_v100(fuente_path, texto):
    ...
    nuevo = _inyectar_motor_rotacion_v100(nuevo)
...
def main():
    ...
    nuevo = construir_v100(fuente, texto)
