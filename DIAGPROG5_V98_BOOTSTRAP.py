...
TARGET_VERSION = "98"
OUTPUT_NAME = "bot_wallapop_profesional_v98_ADMIN_COLA_CAPACIDAD_VISIBLE.py"
...
# =========================================================
# V98 · CAPA DE FLUIDEZ DE INTERFAZ
# =========================================================
...
try:
    ...
    print("[V98] Capa de fluidez dashboard no aplicada:", _e_v89_ui)

"""


def _inyectar_fluidez(texto):
    if "V98 · CAPA DE FLUIDEZ DE INTERFAZ" in texto:
        return texto
...
def _hacer_comprobacion_update_inicio_no_bloqueante(texto):
    ...
    if inicio == -1 or fin == -1:
        return texto
    nuevo = """# Comprobación silenciosa del servidor de actualizaciones (V98, no bloqueante).
...
''',
        '''            (
                ...
                "V98 ya migró automáticamente los publicados.json antiguos. "
...
'''
    ...
    try:
        _diag_log_path = Path(CARPETA_DATOS_USUARIO) / "rotacion_v98.log"
...
def _inyectar_motor_rotacion_v98(texto):
    if "V98 · MOTOR DE ROTACIÓN ROBUSTO" in texto:
        return texto

    codigo = r'''

# =========================================================
# V98 · MOTOR DE ROTACIÓN ROBUSTO
# =========================================================
# Objetivo:
...
_ROTACION_V98_LOCK = threading.Lock()
_ROTACION_V98_PROCESADOS = set()
_ROTACION_V98_ULTIMO = ""
_ROTACION_V98_EN_CURSO = False


def _v98_log_rotacion(texto):
    global _ROTACION_V98_ULTIMO

    _ROTACION_V98_ULTIMO = str(
        texto
    )
...
def _v98_titulo_candidato(item):
    if not isinstance(
        item,
        dict
    ):
...
def _v98_click_visible(page, textos, timeout=2500):
    for texto in textos:
        # Texto visible.
        try:
...
def _v98_normalizar_texto(texto):
    try:
        import unicodedata
...
def _v98_guardar_publicados_compat(datos):
    """
...
def _v98_guardar_url_candidato(candidato, url):
    if not url:
        return
    ...
    try:
        ...
        if (
            ...
            _v98_guardar_publicados_compat(
                datos
            )

            return

        titulo_objetivo = _v98_normalizar_texto(
            _v98_titulo_candidato(
                candidato
            )
        )
...
def _v98_extraer_url_por_titulo(page, titulo):
    objetivo = _v98_normalizar_texto(
        titulo
    )
...
def _v98_descubrir_url_candidato(contexto, candidato):
    titulo = _v98_titulo_candidato(
        candidato
    )
    ...
    try:
        for pagina in list(
            ...
            try:
                url = _v98_extraer_url_por_titulo(
                    pagina,
                    titulo
                )

                if url:
                    _v98_guardar_url_candidato(
                        candidato,
                        url
                    )
...
def _v98_abrir_candidato(page, candidato):
    url = _v98_url_candidato(
        candidato
    )
    ...
    if not url:
        try:
            url = _v98_descubrir_url_candidato(
                page.context,
                candidato
            )
        except Exception as e:
...
def _v98_eliminar_con_motor_existente(candidato):
    """
...
def _v98_dump_controles(page, limite=120):
    """
...
def _v98_click_eliminar_avanzado(page):
    patrones = (
        "eliminar",
        "borrar",
        "delete",
...
def _v98_eliminar_en_wallapop(candidato):
    titulo = _v98_titulo_candidato(
        candidato
    )
    ...
    ok_interno, detalle_interno = _v98_eliminar_con_motor_existente(
        candidato
    )
    ...
    try:
        with sync_playwright() as p:
            ...
            eliminado_click, eliminado_texto = _v98_click_eliminar_avanzado(
                pagina
            )

            if not eliminado_click:
                controles = _v98_dump_controles(
                    pagina
                )
...
def _v98_marcar_local_eliminado(candidato):
    try:
        datos = cargar_publicados()
...
def _v98_rotar_una_vez():
    if not _v98_rotacion_activa():
        return False, (
            "Rotación desactivada."
        )

    candidatos = _v98_candidatos_locales()
    ...
    ok, detalle = _v98_eliminar_en_wallapop(
        candidato
    )
...
def _v98_reactivar_item(item, detalle):
    try:
        item[
            "estado_cola"
        ] = "PENDIENTE"
...
def _v98_worker_limite(item, clave):
    global _ROTACION_V98_EN_CURSO

    if not _ROTACION_V98_LOCK.acquire(
        blocking=False
    ):
        return

    _ROTACION_V98_EN_CURSO = True
    ...
    try:
        ...
        try:
            _ROTACION_V98_LOCK.release()
        except Exception:
            pass


def _v98_vigilar_limite_catalogo():
    try:
        if (
            _v98_rotacion_activa()
            and not _ROTACION_V98_EN_CURSO
        ):
            for indice, item in enumerate(
                ...
                if clave in _ROTACION_V98_PROCESADOS:
                    continue

                try:
                    item[
                        "rotacion_detalle"
                    ] = "V98: límite detectado; buscando candidato seguro..."
...
except Exception as _e_v98_rot:
    print(
        "[ROTACION V98] No pude iniciar watchdog:",
        _e_v98_rot
    )
...
'''
    ...
    if marcador not in texto:
        raise RuntimeError(
            "No pude insertar el motor V98 antes del mainloop."
        )
...
def _inyectar_diagnostico_runtime_v98(texto):
    if "V98 · DIAGNÓSTICO RUNTIME GARANTIZADO" in texto:
        return texto

    codigo = r'''

# =========================================================
# V98 · DIAGNÓSTICO RUNTIME GARANTIZADO
# =========================================================
...
def _v98_serializar_seguro(valor):
    try:
        if isinstance(
            valor,
            (
...
def _v98_candidatos_resumen():
    candidatos = []

    try:
        funcion = globals().get(
            "_v98_candidatos_locales"
        )
...
def _v98_abrir_informe_rotacion(texto_base=""):
    try:
        ...
        partes = [
            ...
            "",
            "================ CANDIDATOS ================",
            _v98_candidatos_resumen(),
            "",
            "================ COLA / ERRORES SIN CORTAR ================",
            _v98_texto_cola_completo(),
            "",
            "================ LOG ROTACIÓN ================",
            _v98_log_rotacion_texto(),
            "",
            "================ SISTEMA ================",
            "Chrome/CDP: "
            + str(
...
def _v98_showinfo(title, message, *args, **kwargs):
    try:
        titulo = str(
            title
        ).lower()
        ...
        if (
            ...
            try:
                ventana.after(
                    0,
                    lambda m=mensaje: _v98_abrir_informe_rotacion(
                        m
                    )
                )
...
messagebox.showinfo = _v98_showinfo
...
def _inyectar_capacidad_cola_v98(texto):
    if "V98 · CAPACIDAD VISIBLE EN BARRA DE COLA" in texto:
        return texto
...
_v98_boton_capacidad = None
_v98_control_instalado = False


def _v98_cargar_capacidad():
    try:
...
def _v98_guardar_capacidad(valor):
...
CAPACIDAD_COLA = _v98_cargar_capacidad()
...
def _v98_actualizar_boton():
    try:
        if _v98_boton_capacidad is not None:
            _v98_boton_capacidad.configure(
                text=f"⚙ Capacidad {_v98_uso_cola()}/{capacidad_cola_actual()}"
            )
    except Exception:
        pass


def _v98_abrir_dialogo_capacidad():
    ...
    def guardar():
        ...
        CAPACIDAD_COLA = _v98_guardar_capacidad(valor)
        _v98_actualizar_boton()
        dialogo.destroy()
...
def _v98_descendientes(widget):
    ...
    for hijo in hijos:
        salida.append(hijo)
        salida.extend(_v98_descendientes(hijo))
    return salida


def _v98_texto(widget):
    try:
...
def _v98_buscar_boton_anadir():
    try:
        todos = _v98_descendientes(ventana)
    except Exception:
        todos = []

    for widget in todos:
        texto = _v98_texto(widget)
        if (
...
def _v98_instalar_boton_capacidad(intentos=0):
    global _v98_boton_capacidad, _v98_control_instalado

    if _v98_control_instalado:
        return

    referencia = _v98_buscar_boton_anadir()

    if referencia is None:
        if intentos < 60:
            ventana.after(
                500,
                lambda: _v98_instalar_boton_capacidad(intentos + 1)
            )
        return
    ...
    try:
        ...
        _v98_boton_capacidad = ctk.CTkButton(
            parent,
            text="",
            command=_v98_abrir_dialogo_capacidad,
            height=32,
            width=155,
            fg_color="#245a86",
            hover_color="#2f6fa3"
        ...
        if gestor == "grid":
            ...
            _v98_boton_capacidad.pack(
                side="left",
                padx=(8, 0)
            )

        _v98_control_instalado = True
        _v98_actualizar_boton()

    except Exception as e:
        print("[V98] Error instalando botón capacidad:", e)
        if intentos < 60:
            ventana.after(
                500,
                lambda: _v98_instalar_boton_capacidad(intentos + 1)
...
def _v98_envuelve_funcion_cola(nombre, funcion):
    if getattr(funcion, "_v98_capacidad_wrapped", False):
        return funcion

    def wrapper(*args, **kwargs):
        actual = _v98_uso_cola()
    ...
    wrapper._v98_capacidad_wrapped = True
    wrapper.__name__ = getattr(funcion, "__name__", nombre)
    return wrapper


def _v98_instalar_limites():
    for nombre, funcion in list(globals().items()):
        ...
        try:
            globals()[nombre] = _v98_envuelve_funcion_cola(nombre, funcion)
        except Exception:
            pass
...
def _v98_tick():
    ...
    if not _v98_control_instalado:
        _v98_instalar_boton_capacidad()

    try:
        ventana.after(1000, _v98_tick)
    except Exception:
        pass


try:
    ventana.after(300, _v98_instalar_boton_capacidad)
    ventana.after(700, _v98_instalar_limites)
    ventana.after(1000, _v98_tick)
except Exception as e:
    print("[V98] No pude iniciar capacidad de cola:", e)
...
def construir_v98(fuente_path, texto):
    ...
    nuevo = _inyectar_motor_rotacion_v98(nuevo)
...
def main():
    ...
    nuevo = construir_v98(fuente, texto)
    ...
    if proceso_nuevo.poll() is not None:
        raise RuntimeError(
            "La V98 se cerró durante el arranque. "
