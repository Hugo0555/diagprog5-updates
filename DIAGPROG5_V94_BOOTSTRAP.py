...
TARGET_VERSION = "94"
OUTPUT_NAME = "bot_wallapop_profesional_v94_ADMIN_ROTACION_URL_FIX.py"
...
# =========================================================
# V94 · CAPA DE FLUIDEZ DE INTERFAZ
# =========================================================
...
try:
    ...
    print("[V94] Capa de fluidez dashboard no aplicada:", _e_v89_ui)

"""


def _inyectar_fluidez(texto):
    if "V94 · CAPA DE FLUIDEZ DE INTERFAZ" in texto:
        return texto
...
def _hacer_comprobacion_update_inicio_no_bloqueante(texto):
    ...
    if inicio == -1 or fin == -1:
        return texto
    nuevo = """# Comprobación silenciosa del servidor de actualizaciones (V94, no bloqueante).
...
''',
        '''            (
                ...
                "V94 ya migró automáticamente los publicados.json antiguos. "
...
'''
    ...
    try:
        _diag_log_path = Path(CARPETA_DATOS_USUARIO) / "rotacion_v94.log"
...
def _inyectar_motor_rotacion_v94(texto):
    if "V94 · MOTOR DE ROTACIÓN ROBUSTO" in texto:
        return texto

    codigo = r'''

# =========================================================
# V94 · MOTOR DE ROTACIÓN ROBUSTO
# =========================================================
# Objetivo:
...
_ROTACION_V94_LOCK = threading.Lock()
_ROTACION_V94_PROCESADOS = set()
_ROTACION_V94_ULTIMO = ""
_ROTACION_V94_EN_CURSO = False


def _v94_log_rotacion(texto):
    global _ROTACION_V94_ULTIMO

    _ROTACION_V94_ULTIMO = str(
        texto
    )
...
def _v94_titulo_candidato(item):
    if not isinstance(
        item,
        dict
    ):
...
def _v94_click_visible(page, textos, timeout=2500):
    for texto in textos:
        # Texto visible.
        try:
...
def _v94_normalizar_texto(texto):
    try:
        import unicodedata
...
def _v94_guardar_url_candidato(candidato, url):
    if not url:
        return
    ...
    try:
        ...
        titulo_objetivo = _v94_normalizar_texto(
            _v94_titulo_candidato(
                candidato
            )
        )
...
def _v94_extraer_url_por_titulo(page, titulo):
    objetivo = _v94_normalizar_texto(
        titulo
    )
...
def _v94_descubrir_url_candidato(contexto, candidato):
    titulo = _v94_titulo_candidato(
        candidato
    )
    ...
    try:
        for pagina in list(
            ...
            try:
                url = _v94_extraer_url_por_titulo(
                    pagina,
                    titulo
                )

                if url:
                    _v94_guardar_url_candidato(
                        candidato,
                        url
                    )
...
def _v94_abrir_candidato(page, candidato):
    url = _v94_url_candidato(
        candidato
    )
    ...
    if not url:
        try:
            url = _v94_descubrir_url_candidato(
                page.context,
                candidato
            )
        except Exception as e:
...
def _v94_eliminar_con_motor_existente(candidato):
    """
...
def _v94_eliminar_en_wallapop(candidato):
    titulo = _v94_titulo_candidato(
        candidato
    )
    ...
    ok_interno, detalle_interno = _v94_eliminar_con_motor_existente(
        candidato
    )
    ...
    try:
        with sync_playwright() as p:
            ...
            eliminado_click = _v94_click_visible(
                pagina,
                (
                    "Eliminar producto",
                    "Eliminar anuncio",
...
def _v94_marcar_local_eliminado(candidato):
    try:
        datos = cargar_publicados()
...
def _v94_rotar_una_vez():
    if not _v94_rotacion_activa():
        return False, (
            "Rotación desactivada."
        )

    candidatos = _v94_candidatos_locales()
    ...
    ok, detalle = _v94_eliminar_en_wallapop(
        candidato
    )
...
def _v94_reactivar_item(item, detalle):
    try:
        item[
            "estado_cola"
        ] = "PENDIENTE"
...
def _v94_worker_limite(item, clave):
    global _ROTACION_V94_EN_CURSO

    if not _ROTACION_V94_LOCK.acquire(
        blocking=False
    ):
        return

    _ROTACION_V94_EN_CURSO = True
    ...
    try:
        ...
        try:
            _ROTACION_V94_LOCK.release()
        except Exception:
            pass


def _v94_vigilar_limite_catalogo():
    try:
        if (
            _v94_rotacion_activa()
            and not _ROTACION_V94_EN_CURSO
        ):
            for indice, item in enumerate(
                ...
                if clave in _ROTACION_V94_PROCESADOS:
                    continue

                try:
                    item[
                        "rotacion_detalle"
                    ] = "V94: límite detectado; buscando candidato seguro..."
...
except Exception as _e_v94_rot:
    print(
        "[ROTACION V94] No pude iniciar watchdog:",
        _e_v94_rot
    )
...
'''
    ...
    if marcador not in texto:
        raise RuntimeError(
            "No pude insertar el motor V94 antes del mainloop."
        )
...
def _inyectar_diagnostico_runtime_v94(texto):
    if "V94 · DIAGNÓSTICO RUNTIME GARANTIZADO" in texto:
        return texto

    codigo = r'''

# =========================================================
# V94 · DIAGNÓSTICO RUNTIME GARANTIZADO
# =========================================================
...
_DIAG_V94_SHOWINFO_ORIGINAL = messagebox.showinfo


def _v94_serializar_seguro(valor):
    try:
        if isinstance(
            valor,
            (
...
def _v94_candidatos_resumen():
    candidatos = []

    try:
        funcion = globals().get(
            "_v94_candidatos_locales"
        )
...
def _v94_abrir_informe_rotacion(texto_base=""):
    try:
        ...
        ventana_diag.title(
            "DIAGPROG5 · Diagnóstico total de rotación V94"
        )
        ...
        cabecera = ctk.CTkLabel(
            marco,
            text="DIAGNÓSTICO TOTAL DE ROTACIÓN · V94",
            font=(
                "Segoe UI",
                18,
                "bold"
        ...
        partes = [
            ...
            "",
            "================ CANDIDATOS ================",
            _v94_candidatos_resumen(),
            "",
            "================ COLA / ERRORES SIN CORTAR ================",
            _v94_texto_cola_completo(),
            "",
            "================ LOG ROTACIÓN ================",
            _v94_log_rotacion_texto(),
            "",
            "================ SISTEMA ================",
            "Chrome/CDP: "
            + str(
        ...
        print(
            "[V94 DIAG] No pude abrir informe total:",
            e
        )
...
def _v94_showinfo(title, message, *args, **kwargs):
    try:
        titulo = str(
            title
        ).lower()
        ...
        if (
            ...
            try:
                ventana.after(
                    0,
                    lambda m=mensaje: _v94_abrir_informe_rotacion(
                        m
                    )
                )
    ...
    return _DIAG_V94_SHOWINFO_ORIGINAL(
        title,
        message,
        *args,
        **kwargs
...
messagebox.showinfo = _v94_showinfo
...
def construir_v94(fuente_path, texto):
    ...
    nuevo = _inyectar_diagnostico_runtime_v94(nuevo)
    ...
    nuevo = _inyectar_motor_rotacion_v94(nuevo)
...
def main():
    ...
    nuevo = construir_v94(fuente, texto)
