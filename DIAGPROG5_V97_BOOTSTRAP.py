...
TARGET_VERSION = "97"
OUTPUT_NAME = "bot_wallapop_profesional_v97_ADMIN_COLA_PRO.py"
...
# =========================================================
# V97 · CAPA DE FLUIDEZ DE INTERFAZ
# =========================================================
...
try:
    ...
    print("[V97] Capa de fluidez dashboard no aplicada:", _e_v89_ui)

"""


def _inyectar_fluidez(texto):
    if "V97 · CAPA DE FLUIDEZ DE INTERFAZ" in texto:
        return texto
...
def _hacer_comprobacion_update_inicio_no_bloqueante(texto):
    ...
    if inicio == -1 or fin == -1:
        return texto
    nuevo = """# Comprobación silenciosa del servidor de actualizaciones (V97, no bloqueante).
...
''',
        '''            (
                ...
                "V97 ya migró automáticamente los publicados.json antiguos. "
...
'''
    ...
    try:
        _diag_log_path = Path(CARPETA_DATOS_USUARIO) / "rotacion_v97.log"
...
def _inyectar_motor_rotacion_v97(texto):
    if "V97 · MOTOR DE ROTACIÓN ROBUSTO" in texto:
        return texto

    codigo = r'''

# =========================================================
# V97 · MOTOR DE ROTACIÓN ROBUSTO
# =========================================================
# Objetivo:
...
_ROTACION_V97_LOCK = threading.Lock()
_ROTACION_V97_PROCESADOS = set()
_ROTACION_V97_ULTIMO = ""
_ROTACION_V97_EN_CURSO = False


def _v97_log_rotacion(texto):
    global _ROTACION_V97_ULTIMO

    _ROTACION_V97_ULTIMO = str(
        texto
    )
...
def _v97_titulo_candidato(item):
    if not isinstance(
        item,
        dict
    ):
...
def _v97_click_visible(page, textos, timeout=2500):
    for texto in textos:
        # Texto visible.
        try:
...
def _v97_normalizar_texto(texto):
    try:
        import unicodedata
...
def _v97_guardar_publicados_compat(datos):
    """
...
def _v97_guardar_url_candidato(candidato, url):
    if not url:
        return
    ...
    try:
        ...
        if (
            ...
            _v97_guardar_publicados_compat(
                datos
            )

            return

        titulo_objetivo = _v97_normalizar_texto(
            _v97_titulo_candidato(
                candidato
            )
        )
...
def _v97_extraer_url_por_titulo(page, titulo):
    objetivo = _v97_normalizar_texto(
        titulo
    )
...
def _v97_descubrir_url_candidato(contexto, candidato):
    titulo = _v97_titulo_candidato(
        candidato
    )
    ...
    try:
        for pagina in list(
            ...
            try:
                url = _v97_extraer_url_por_titulo(
                    pagina,
                    titulo
                )

                if url:
                    _v97_guardar_url_candidato(
                        candidato,
                        url
                    )
...
def _v97_abrir_candidato(page, candidato):
    url = _v97_url_candidato(
        candidato
    )
    ...
    if not url:
        try:
            url = _v97_descubrir_url_candidato(
                page.context,
                candidato
            )
        except Exception as e:
...
def _v97_eliminar_con_motor_existente(candidato):
    """
...
def _v97_dump_controles(page, limite=120):
    """
...
def _v97_click_eliminar_avanzado(page):
    patrones = (
        "eliminar",
        "borrar",
        "delete",
...
def _v97_eliminar_en_wallapop(candidato):
    titulo = _v97_titulo_candidato(
        candidato
    )
    ...
    ok_interno, detalle_interno = _v97_eliminar_con_motor_existente(
        candidato
    )
    ...
    try:
        with sync_playwright() as p:
            ...
            eliminado_click, eliminado_texto = _v97_click_eliminar_avanzado(
                pagina
            )

            if not eliminado_click:
                controles = _v97_dump_controles(
                    pagina
                )
...
def _v97_marcar_local_eliminado(candidato):
    try:
        datos = cargar_publicados()
...
def _v97_rotar_una_vez():
    if not _v97_rotacion_activa():
        return False, (
            "Rotación desactivada."
        )

    candidatos = _v97_candidatos_locales()
    ...
    ok, detalle = _v97_eliminar_en_wallapop(
        candidato
    )
...
def _v97_reactivar_item(item, detalle):
    try:
        item[
            "estado_cola"
        ] = "PENDIENTE"
...
def _v97_worker_limite(item, clave):
    global _ROTACION_V97_EN_CURSO

    if not _ROTACION_V97_LOCK.acquire(
        blocking=False
    ):
        return

    _ROTACION_V97_EN_CURSO = True
    ...
    try:
        ...
        try:
            _ROTACION_V97_LOCK.release()
        except Exception:
            pass


def _v97_vigilar_limite_catalogo():
    try:
        if (
            _v97_rotacion_activa()
            and not _ROTACION_V97_EN_CURSO
        ):
            for indice, item in enumerate(
                ...
                if clave in _ROTACION_V97_PROCESADOS:
                    continue

                try:
                    item[
                        "rotacion_detalle"
                    ] = "V97: límite detectado; buscando candidato seguro..."
...
except Exception as _e_v97_rot:
    print(
        "[ROTACION V97] No pude iniciar watchdog:",
        _e_v97_rot
    )
...
'''
    ...
    if marcador not in texto:
        raise RuntimeError(
            "No pude insertar el motor V97 antes del mainloop."
        )
...
def _limpiar_motores_rotacion_generados(texto):
    """
    V94: elimina motores/diagnósticos runtime inyectados por V91-V93 para
    evitar que dos watchdogs intenten rotar el catálogo a la vez.
    """
    patrones = [
        r'\n# =========================================================\n# V(?:91|92|93)(?:\.\d+)? · MOTOR DE ROTACIÓN ROBUSTO\n# =========================================================[\s\S]*?(?=\n# =========================================================\n# V\d+(?:\.\d+)? · |\nventana\.mainloop\(\))',
        r'\n# =========================================================\n# V(?:91|92|93)(?:\.\d+)? · DIAGNÓSTICO RUNTIME GARANTIZADO\n# =========================================================[\s\S]*?(?=\n# =========================================================\n# V\d+(?:\.\d+)? · |\nventana\.mainloop\(\))',
    ]
...
def _inyectar_diagnostico_runtime_v97(texto):
    if "V97 · DIAGNÓSTICO RUNTIME GARANTIZADO" in texto:
        return texto

    codigo = r'''

# =========================================================
# V97 · DIAGNÓSTICO RUNTIME GARANTIZADO
# =========================================================
...
def _v97_serializar_seguro(valor):
    try:
        if isinstance(
            valor,
            (
...
def _v97_candidatos_resumen():
    candidatos = []

    try:
        funcion = globals().get(
            "_v97_candidatos_locales"
        )
...
def _v97_abrir_informe_rotacion(texto_base=""):
    try:
        ...
        partes = [
            ...
            "",
            "================ CANDIDATOS ================",
            _v97_candidatos_resumen(),
            "",
            "================ COLA / ERRORES SIN CORTAR ================",
            _v97_texto_cola_completo(),
            "",
            "================ LOG ROTACIÓN ================",
            _v97_log_rotacion_texto(),
            "",
            "================ SISTEMA ================",
            "Chrome/CDP: "
            + str(
...
def _v97_showinfo(title, message, *args, **kwargs):
    try:
        titulo = str(
            title
        ).lower()
        ...
        if (
            ...
            try:
                ventana.after(
                    0,
                    lambda m=mensaje: _v97_abrir_informe_rotacion(
                        m
                    )
                )
...
messagebox.showinfo = _v97_showinfo
...
def _inyectar_capacidad_cola_v97(texto):
    if "V97 · CAPACIDAD CONFIGURABLE DE COLA" in texto:
        return texto

    codigo = r'''

# =========================================================
# V97 · CAPACIDAD CONFIGURABLE DE COLA
# =========================================================
_ARCHIVO_CAPACIDAD_COLA = Path(CARPETA_DATOS_USUARIO) / "capacidad_cola.json"
_CAPACIDAD_COLA_DEFECTO = 500
_CAPACIDAD_COLA_MIN = 1
_CAPACIDAD_COLA_MAX = 5000

_v97_panel_capacidad_creado = False
_v97_entry_capacidad = None
_v97_label_capacidad = None


def _v97_cargar_capacidad_cola():
    try:
...
def _v97_guardar_capacidad_cola(valor):
    ...
    _ARCHIVO_CAPACIDAD_COLA.write_text(
        json.dumps({"capacidad": valor, "actualizado": time.strftime("%d/%m/%Y %H:%M:%S")}, ensure_ascii=False, indent=2),
        encoding="utf-8"
    )
    return valor


CAPACIDAD_COLA = _v97_cargar_capacidad_cola()
...
def _v97_descendientes(widget):
    ...
    for hijo in hijos:
        salida.append(hijo)
        salida.extend(_v97_descendientes(hijo))
    return salida


def _v97_texto_widget(widget):
    try:
...
def _v97_buscar_panel_cola():
    for nombre in (
        "queue_card", "cola_card", "frame_cola", "cola_frame",
        "panel_cola", "tab_cola", "pagina_cola", "page_cola", "content_cola"
    ):
    ...
    try:
        todos = [ventana] + _v97_descendientes(ventana)
    except Exception:
        todos = []

    for widget in todos:
        texto = _v97_texto_widget(widget).lower()
        if "cola de publicación" in texto or "cola de publicacion" in texto:
            try:
    ...
    for widget in todos:
        try:
            textos = [_v97_texto_widget(x).lower() for x in _v97_descendientes(widget)[:80]]
            blob = " | ".join(t for t in textos if t)
            if (("añadir anuncio" in blob or "anadir anuncio" in blob) and ("vaciar cola" in blob or "iniciar cola" in blob)):
                return widget
        except Exception:
            pass
...
def _v97_refrescar_estado_capacidad():
    try:
        actual = len(cola_anuncios)
    except Exception:
        actual = 0
    try:
        if _v97_label_capacidad is not None:
            _v97_label_capacidad.configure(text=f"Uso actual: {actual} / {capacidad_cola_actual()}")
    except Exception:
        pass


def _v97_guardar_capacidad_desde_ui():
    global CAPACIDAD_COLA
    if _v97_entry_capacidad is None:
        return

    try:
        valor = int(str(_v97_entry_capacidad.get()).strip())
    except Exception:
        messagebox.showerror("Capacidad de cola", "Escribe un número entero entre 1 y 5000.")
        return

    if valor < 1 or valor > 5000:
        messagebox.showerror("Capacidad de cola", "La capacidad debe estar entre 1 y 5000.")
        return

    CAPACIDAD_COLA = _v97_guardar_capacidad_cola(valor)
    _v97_refrescar_estado_capacidad()
    messagebox.showinfo("Capacidad de cola", f"Capacidad guardada: {CAPACIDAD_COLA} anuncios.")


def _v97_crear_panel_capacidad(parent):
    global _v97_panel_capacidad_creado, _v97_entry_capacidad, _v97_label_capacidad

    if _v97_panel_capacidad_creado:
        return True

    try:
        panel = ctk.CTkFrame(parent, fg_color="#081017", corner_radius=10, border_width=1, border_color="#7d1422")
        try:
            panel.pack(fill="x", padx=10, pady=(8, 10))
        except Exception:
            panel.grid(row=99, column=0, columnspan=20, sticky="ew", padx=10, pady=(8, 10))

        ctk.CTkLabel(panel, text="Capacidad de la cola", font=("Segoe UI", 12, "bold"), anchor="w").pack(fill="x", padx=12, pady=(10, 2))
        ctk.CTkLabel(panel, text="Máximo de anuncios que puede contener la cola (1–5000).", font=("Segoe UI", 9), text_color="#98a7b2", anchor="w").pack(fill="x", padx=12, pady=(0, 8))

        fila = ctk.CTkFrame(panel, fg_color="transparent")
        fila.pack(fill="x", padx=12, pady=(0, 10))

        _v97_entry_capacidad = ctk.CTkEntry(fila, height=36)
        _v97_entry_capacidad.pack(side="left", fill="x", expand=True, padx=(0, 8))
        _v97_entry_capacidad.insert(0, str(capacidad_cola_actual()))

        ctk.CTkButton(fila, text="Guardar capacidad", command=_v97_guardar_capacidad_desde_ui, height=36).pack(side="left", padx=(0, 8))

        _v97_label_capacidad = ctk.CTkLabel(fila, text="", font=("Segoe UI", 10, "bold"))
        _v97_label_capacidad.pack(side="right")

        _v97_panel_capacidad_creado = True
        _v97_refrescar_estado_capacidad()
        return True
    except Exception as e:
        print("[V97] Error creando panel de capacidad:", e)
        return False


def _v97_instalar_control_capacidad(intentos=0):
    if _v97_panel_capacidad_creado:
        return

    parent = _v97_buscar_panel_cola()

    if parent is not None and _v97_crear_panel_capacidad(parent):
        print("[V97] Capacidad de cola visible.")
        return

    if intentos < 40:
        try:
            ventana.after(500, lambda: _v97_instalar_control_capacidad(intentos + 1))
        except Exception:
            pass
    else:
        print("[V97] No pude localizar el panel Cola tras 20 segundos.")


def _v97_envuelve_funcion_cola(nombre, funcion):
    if getattr(funcion, "_v97_capacidad_wrapped", False):
        return funcion
    ...
    def wrapper(*args, **kwargs):
        ...
        if actual >= limite:
            messagebox.showwarning("Cola llena", f"La cola ha alcanzado {actual} / {limite} anuncios.")
            return None
        ...
        try:
            if len(cola_anuncios) > limite:
                ...
                for nombre_guardar in ("guardar_cola_en_disco", "guardar_cola"):
                    fn = globals().get(nombre_guardar)
                    if callable(fn):
                        try:
                            fn()
                            break
                ...
                fn_refrescar = globals().get("refrescar_cola")
                if callable(fn_refrescar):
                    try:
                        fn_refrescar()
                    except Exception:
                        pass
    ...
    wrapper._v97_capacidad_wrapped = True
    wrapper.__name__ = getattr(funcion, "__name__", nombre)
    return wrapper


def _v97_instalar_limite_funciones_cola():
    for nombre, funcion in list(globals().items()):
        ...
        try:
            globals()[nombre] = _v97_envuelve_funcion_cola(nombre, funcion)
        except Exception:
            pass


def _v97_tick_capacidad():
    _v97_refrescar_estado_capacidad()

    if not _v97_panel_capacidad_creado:
        _v97_instalar_control_capacidad()

    try:
        ventana.after(1000, _v97_tick_capacidad)
    except Exception:
        pass


try:
    ventana.after(300, _v97_instalar_control_capacidad)
    ventana.after(700, _v97_instalar_limite_funciones_cola)
    ventana.after(1000, _v97_tick_capacidad)
except Exception as _e_v97_capacidad:
    print("[V97] No pude iniciar capacidad de cola:", _e_v97_capacidad)
...
def construir_v97(fuente_path, texto):
    ...
    nuevo = _inyectar_motor_rotacion_v97(nuevo)
...
def main():
    ...
    nuevo = construir_v97(fuente, texto)
    ...
    if proceso_nuevo.poll() is not None:
        raise RuntimeError(
            "La V97 se cerró durante el arranque. "
