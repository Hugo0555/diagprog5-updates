...
TARGET_VERSION = "90"
OUTPUT_NAME = "bot_wallapop_profesional_v90_ADMIN_ROTACION_ESTABLE.py"
...
# =========================================================
# V90 · CAPA DE FLUIDEZ DE INTERFAZ
# =========================================================
...
try:
    ...
    print("[V90] Capa de fluidez dashboard no aplicada:", _e_v89_ui)

"""


def _inyectar_fluidez(texto):
    if "V90 · CAPA DE FLUIDEZ DE INTERFAZ" in texto:
        return texto
...
def _hacer_comprobacion_update_inicio_no_bloqueante(texto):
    ...
    if inicio == -1 or fin == -1:
        return texto
    nuevo = """# Comprobación silenciosa del servidor de actualizaciones (V90, no bloqueante).
...
''',
        '''            (
                ...
                "V90 ya migró automáticamente los publicados.json antiguos. "
...
def _inyectar_motor_rotacion_v90(texto):
    if "V90 · MOTOR DE ROTACIÓN ROBUSTO" in texto:
        return texto

    codigo = r'''

# =========================================================
# V90 · MOTOR DE ROTACIÓN ROBUSTO
# =========================================================
# Objetivo:
...
_ROTACION_V90_LOCK = threading.Lock()
_ROTACION_V90_PROCESADOS = set()
_ROTACION_V90_ULTIMO = ""
_ROTACION_V90_EN_CURSO = False


def _v90_log_rotacion(texto):
    global _ROTACION_V90_ULTIMO

    _ROTACION_V90_ULTIMO = str(
        texto
    )
...
def _v90_titulo_candidato(item):
    if not isinstance(
        item,
        dict
    ):
...
def _v90_click_visible(page, textos, timeout=2500):
    for texto in textos:
        # Texto visible.
        try:
...
def _v90_abrir_candidato(page, candidato):
    url = _v90_url_candidato(
        candidato
    )

    titulo = _v90_titulo_candidato(
        candidato
    )
...
def _v90_eliminar_en_wallapop(candidato):
    titulo = _v90_titulo_candidato(
        candidato
    )
    ...
    try:
        with sync_playwright() as p:
            ...
            eliminado_click = _v90_click_visible(
                pagina,
                (
                    "Eliminar producto",
                    "Eliminar anuncio",
...
def _v90_marcar_local_eliminado(candidato):
    try:
        datos = cargar_publicados()
...
def _v90_rotar_una_vez():
    if not _v90_rotacion_activa():
        return False, (
            "Rotación desactivada."
        )

    candidatos = _v90_candidatos_locales()
    ...
    ok, detalle = _v90_eliminar_en_wallapop(
        candidato
    )
...
def _v90_reactivar_item(item, detalle):
    try:
        item[
            "estado_cola"
        ] = "PENDIENTE"
...
def _v90_worker_limite(item, clave):
    global _ROTACION_V90_EN_CURSO

    if not _ROTACION_V90_LOCK.acquire(
        blocking=False
    ):
        return

    _ROTACION_V90_EN_CURSO = True
    ...
    try:
        ...
        try:
            _ROTACION_V90_LOCK.release()
        except Exception:
            pass


def _v90_vigilar_limite_catalogo():
    try:
        if (
            _v90_rotacion_activa()
            and not _ROTACION_V90_EN_CURSO
        ):
            for indice, item in enumerate(
                ...
                if clave in _ROTACION_V90_PROCESADOS:
                    continue

                threading.Thread(
                    target=_v90_worker_limite,
                    args=(
                        item,
                        clave,
                    ),
...
except Exception as _e_v90_rot:
    print(
        "[ROTACION V90] No pude iniciar watchdog:",
        _e_v90_rot
    )
...
'''
    ...
    if marcador not in texto:
        raise RuntimeError(
            "No pude insertar el motor V90 antes del mainloop."
        )
...
def construir_v90(fuente_path, texto):
    ...
    nuevo = _inyectar_motor_rotacion_v90(nuevo)
...
def main():
    ...
    nuevo = construir_v90(fuente, texto)
